@echo off
rem Build umvc3_shaderdump.asi (an ordinary x64 DLL with an .asi extension).
setlocal
...
cl /nologo /LD /O2 /EHsc /W4 /DNDEBUG /std:c++17 ^
   umvc3_shaderdump.cpp ^
   /Fo:build\ /Fd:build\ ^
   /link /OUT:build\umvc3_shaderdump.asi kernel32.lib user32.lib
set RC=%errorlevel%
popd
if %RC% neq 0 ( echo BUILD FAILED & exit /b %RC% )
echo.
echo Built: %~dp0build\umvc3_shaderdump.asi
exit /b 0
